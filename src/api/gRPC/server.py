...
class Service(article_pb2_grpc.APIServicer):
    ...
    @log_handler
    async def CreateArticle(self, request, context: ServicerContext) -> article_pb2.ArticleResponse:
    ...
    async def GetArticle(self, request, context: ServicerContext):
        article = service.ArticleService.get_article_by_uuid(
            uuid = request.article_uuid
        )
    ...
    async def GetMyArticles(self, request, context: ServicerContext):
        active_user = get_active_user(request.token)
        articles = service.ArticleService.get_articles(
            active_user.uuid
        )
        
        parsed_articles = []
        for art in articles:
            parsed_articles.append(self.__convert_article_to_grpc_response(art.convert_to_schema()))
    ...
    async def SearchArticle(self, request, context: ServicerContext):
        ...
        grpc_response = [self.__convert_article_docs_to_grpc_response(i) for i in articles]
    
        return article_pb2.ArticlesDocsResponse(
            articles = grpc_response
        )
    ...
    async def GetLikedArticles(self, request, context: ServicerContext):
        ...
        liked_articles = service.LikeService.get_likes_articles(
            active_user.uuid
        )
        grpc_response = [self.__convert_article_to_grpc_response(i) for i in liked_articles]
        
        return article_pb2.ArticlesResponse(
            articles = grpc_response
        )
    ...
    async def GetComments(self, request, context: ServicerContext):
        comments = await service.CommentsMongoService.get_comments(
            request.article_uuid
        )
        
        comments = [service.CommentsMongoService.convert_dict_to_schema(i) for i in comments]
        
        grpc_response = [self.__convert_comment_to_grpc_response(c) for c in comments]
        return article_pb2.CommentsResponse(
            comments = grpc_response
        )
    ...
    def __convert_article_to_grpc_response(self, article_schema: schemas.ArticleSchema) -> article_pb2.ArticleResponse:
    ...
    def __convert_article_docs_to_grpc_response(self, article_docs: schemas.ArticleDocumentSchema) -> article_pb2.ArticleDocumentResponse:
    ...
    def __convert_detail_to_grpc_response(self, detail: str):
        return article_pb2.DetailResponse(
            detail = detail
        )
