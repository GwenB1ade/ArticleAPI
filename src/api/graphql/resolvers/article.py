...
class Query:
    ...
    def get_article_by_uuid(self, uuid: str) -> types.ArticleType:
        article = ArticleService.get_article_by_uuid(
            uuid = uuid
        )
    ...
    def get_my_articles(
        ...
        list_articles = []
        for art in articles:
            list_articles.append(types.ArticleType.from_pydantic(art.convert_to_schema()))
...
class Mutation:
    ...
    async def delete_article(
        ...
        return types.DetailResponseType(
            detail = 'The article has been deleted'
        )
