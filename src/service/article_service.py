...
class ArticleService(BaseServices):
    ...
    @classmethod
    def create_article(cls, data: CreateArticleSchema, author_uuid: str) -> ArticleModel:
        """Создание статьи"""
    ...
    @classmethod
    def get_article_by_uuid(cls, uuid: str) -> ArticleModel:
        """Получение статьи по UUID"""
        with session_creater() as s:
            article = s.query(cls.model).filter_by(
                uuid = uuid
            ).first()
    ...
    @classmethod
    def get_articles(cls, author_uuid: str, page: int = 1, page_size: int = None) -> list[ArticleModel]:
        """
        ...
        with session_creater() as s:
            if page_size:
                ...
                articles = s.query(cls.model).filter(
                    cls.model.author_uuid == author_uuid
    ...
    @classmethod
    def get_articles_by_username(cls, author_username: str) -> list[ArticleModel]:
